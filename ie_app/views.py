...
class ExportView(View):
    def get(self, request, model_name: str, with_data=1, *args, **kwargs):
        ...
        try:
            ...
            resource = resources.resources.modelresource_factory(model=model, resource_class=resource_class)
        except KeyError:
            resource = resources.resources.modelresource_factory(model=model, resource_class=resource_classes["default"])
...
class ConfirmImportView(mixins.BaseImportMixin, FormView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({
            'format': self.request.POST.get('format'),  # Utilisez 'format' au lieu de 'input_format'
        })
        return kwargs
    ...
    def form_valid(self, form):
        ...
        try:
            resource = resources.resources.modelresource_factory(
                model=model, resource_class=resource_classes.get(model.__name__, resource_classes["default"])
            )
        except KeyError:
            resource = resources.resources.modelresource_factory(
                model=model, resource_class=resource_classes["default"]
        ...
        if not resource:
            messages.error(self.request, "Impossible d'importer les données de cette table.")
            return redirect(self.request.META.get("HTTP_REFERER"))
        ...
        # Vérifiez que le fichier est un fichier zip valide
        try:
            with zipfile.ZipFile(tmp_storage.name) as zf:
                pass
        except zipfile.BadZipFile:
            messages.error(self.request, "Le fichier n'est pas un fichier Excel valide.")
            return redirect(self.request.META.get("HTTP_REFERER"))
        dataset = input_format.create_dataset(data)
...
class SomeImportView(mixins.BaseImportMixin, FormView):
